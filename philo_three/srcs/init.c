...
/*   Updated: 2021/04/25 14:44:00 by rtrant           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			init_semaphores(t_philo_config *conf)
{
	...
	if ((g_forks_sem = sem_open("g_forks_sem", O_CREAT | O_EXCL, 0644,
				conf->ph_count)) == SEM_FAILED)
	{
		sem_close(g_status_sem);
	...
	if ((g_fullness_sem =
		sem_open("g_fullness_sem", O_CREAT | O_EXCL, 0644, 1)) == SEM_FAILED)
	{
		sem_close(g_status_sem);
		sem_close(g_write_sem);
